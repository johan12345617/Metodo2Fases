function jsx_funcionObjetivo() {
  if (arguments.length < 2) {
    alert(
      "N\xfamero de parametros incorrecto al crear un objeto de tipo funci\xf3n objetivo."
    );
    return null;
  }
  if (
    arguments[0].toLowerCase() != "max" &&
    arguments[0].toLowerCase() != "min"
  ) {
    alert(
      "El primer par\xe1metro de una funci\xf3n objetivo debe ser 'max' o 'min'."
    );
    return null;
  }
  ...
  if (arguments[1] instanceof Array) {
    ...
    for (var i = 0; i < arguments[1].length; i++) {
      if (arguments[1][i] instanceof Racional) {
        ...
        alert(
          "Par\xe1metro incorrecto al crear una funci\xf3n objetivo, valor no num\xe9rico."
        );
        return null;
      }
    }
    ...
    alert(
      "El segundo par\xe1metro de una funci\xf3n objetivo debe ser un array de enteros o un entero."
    );
    return null;
  }
}
...
function jsx_restriccion() {
  if (arguments.length < 3) {
    alert(
      "N\xfamero de par\xe1metros incorrecto al crear un objeto de tipo restricci\xf3n."
    );
    return null;
  }
  this.funcion = new Array();
  if (arguments[0] instanceof Array) {
    for (var i = 0; i < arguments[0].length; i++) {
      if (isNaN(parseFloat(arguments[0][i]))) {
        alert(
          "Par\xe1metro incorrecto al crear un objeto de tipo restricci\xf3n."
        );
        return null;
      }
  ...
  if (
    ...
    alert(
      "Error al crear una restricci\xf3n, el pen\xfaltimo argumento debe ser '<=' o '>='."
    );
  }
  this.signo = arguments[arguments.length - 2];
  if (isNaN(parseFloat(arguments[arguments.length - 1]))) {
    alert(
      "Error al crear una restricci\xf3n, el \xfaltimo argumento debe ser de tipo num\xe9rico."
    );
  }
...
jsx_problema.prototype.variablesHolgura = function () {
  ...
  var n_fo = new jsx_funcionObjetivo(
    this.funcionObjetivo.getTipo(),
    n_fo_param
  );
  ...
  for (var i = 0; i < this.restricciones.length; i++) {
    ...
    n_res[i] = new jsx_restriccion(
      n_res_param,
      "=",
      this.restricciones[i].getLimite()
    );
  }
...
jsx_problema.prototype.setFuncionObjetivo = function () {
  if (arguments.length != 1) {
    alert(
      "La funci\xf3n setFuncionObjetivo de la clase problemas s\xf3lo recibe un par\xe1metro."
    );
    return null;
  }
...
jsx_problema.prototype.addRestriccion = function () {
  if (arguments.length != 1) {
    alert(
      "La funci\xf3n addRestriccion de la clase problemas recibe un s\xf3lo par\xe1metro."
    );
    return;
  }
...
jsx_problema.prototype.getRestriccion = function () {
  if (arguments.length != 1 || isNaN(parseInt(arguments[0]))) {
    alert(
      "Error el metodo getRestriccion de un problema recibe un s\xf3lo par\xe1metro num\xe9rico."
    );
    return null;
  }
...
jsx_problema.prototype.toHTML = function () {
  var res = "<table>";
  res +=
    "<tr><td>" + this.getFuncionObjetivo().toHTML().split(" ")[0] + "</td>";
  res +=
    "<td>" + this.getFuncionObjetivo().toHTML().split(" ")[1] + "</td></tr>";
  for (var i = 0; i < this.numRestricciones(); i++) {
...
jsx_problema.prototype.clone = function () {
  ...
  var nuevafuncionobjetivo = new jsx_funcionObjetivo(
    this.funcionObjetivo.getTipo(),
    this.funcionObjetivo.getFuncion()
  );
  nuevoproblema.setFuncionObjetivo(nuevafuncionobjetivo);
  for (var i = 0; i < this.restricciones.length; i++) {
    var nuevarestriccion = new jsx_restriccion(
      this.restricciones[i].getFuncion(),
      this.restricciones[i].getSigno(),
      this.restricciones[i].getLimite().clone()
    );
...
function jsx_matriz() {
  if (arguments.length != 1 && arguments.length != 2) {
    alert(
      "N\xfamero de par\xe1metros incorrecto al crear un objeto de tipo matriz."
    );
  }
  if (!(arguments[0] instanceof jsx_problema)) {
    alert(
      "El argumento al crear una matriz debe ser un problema (jsx_problema)."
    );
  } else {
    ...
    for (var i = 0; i < this.variables; i++) {
      this.v_costes[i] = new Racional(
        arguments[0].getFuncionObjetivo().getFuncion()[i]
      );
    }
    this.v_costes[this.variables] = new Racional();
    var nvariables =
      arguments[0].noartificiales != 0
        ? arguments[0].noartificiales
        : arguments[0].numVariables();
    ...
    for (var i = 0; i < this.restricciones; i++) {
      ...
      for (var j = 0; j < this.variables; j++) {
        this.matriz[i][j] = new Racional(
          arguments[0].getRestriccion(i).getFuncion()[j]
        );
      }
      this.matriz[i][this.variables] = arguments[0]
        .getRestriccion(i)
        .getLimite();
    }
...
jsx_matriz.prototype.calcularCostesReducidos = function () {
  for (var i = 0; i < this.variables + 1; i++) {
    ...
    for (var j = 0; j < this.restricciones; j++) {
      suma = suma.sumar(
        this.v_costes[this.v_solucion[j]].clone().multiplicar(this.matriz[j][i])
      );
    }
...
jsx_matriz.prototype.quienSaleX = function () {
  ...
  if (this.v_solucion[this.quienSale()] >= this.numvariablesconh) {
    ...
    numero = 1;
    for (
      var k = this.numvariablesconh;
      k < this.v_solucion[this.quienSale()];
      k++
    ) {
...
jsx_matriz.prototype.toString = function () {
  var res = "";
  var resTable = "";
  res += '<div class="jsx_matriz_sol"> ';
  resTable += "<b>con los valores de:</b><br>";
  for (var i = 0; i < this.matriz[0].length - 2; i++) {
    ...
    resTable +=
      $("#jsx_fo_numvar_n").attr("value") > i 
        ? "x<sub>" + (i + 1) + "</sub>=" + valor + "<br>"
        : " ";
  }
  var resultado = 0;
  if (this.usamosM) {
    resultado =
      this.v_costes_reducidosM[this.v_costes_reducidosM.length - 1].toString();
  } else {
    resultado =
      this.v_costes_reducidos[this.v_costes_reducidos.length - 1].toString();
  }
  res += "<b>El valor optimo es:</b> <br> Z=" + resultado + "<br>";
  res += resTable;
  res += "</div>";
  console.log($("#jsx_fo_numvar_n").attr("value"));
  return res;
};
